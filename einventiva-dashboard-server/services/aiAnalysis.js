...
- "services" are user-defined service checks (HTTP endpoints, TCP ports, containers, commands). "ok": true passed, false failed ("error" says why). "ok": null means the check has NO recorded result yet (or is paused) — health UNKNOWN, never report it as down. "probedFrom": "dashboard" means the probe ran over the public network; any other value is the server the probe ran on. "uptime24hPct" is the share of runs that passed in the last 24h. A failing service check on an otherwise healthy machine points to an application-level problem — that contrast is worth calling out explicitly.
